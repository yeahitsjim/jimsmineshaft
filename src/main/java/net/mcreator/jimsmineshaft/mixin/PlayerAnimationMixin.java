	...
	public void setupAnim(PlayerRenderState renderState, CallbackInfo ci) {
		...
		if (!master.equals("jimsmineshaft")) {
			if (!JimsmineshaftModPlayerAnimationAPI.animations.isEmpty())
				JimsmineshaftModPlayerAnimationAPI.animations.clear();
			return;
		}
		PlayerModel model = (PlayerModel) (Object) this;
		...
		if (data.getBoolean("ResetPlayerAnimation")) {
			...
			JimsmineshaftModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		JimsmineshaftModPlayerAnimationAPI.PlayerAnimation animation = JimsmineshaftModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = JimsmineshaftModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				JimsmineshaftMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			JimsmineshaftModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, JimsmineshaftModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			JimsmineshaftModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
