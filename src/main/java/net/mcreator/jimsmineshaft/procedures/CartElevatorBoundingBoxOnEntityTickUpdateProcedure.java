...
public class CartElevatorBoundingBoxOnEntityTickUpdateProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		}
		{
			...
			for (Entity entityiterator : world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(10 / 2d), e -> true).stream().sorted(Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_center))).toList()) {
				...
				if (entityiterator instanceof CartelevatorEastEntity) {
					{
						Entity _ent = entity;
						_ent.teleportTo((entityiterator.getX()), (entityiterator.getY() - 1), (entityiterator.getZ()));
						if (_ent instanceof ServerPlayer _serverPlayer)
				...
				if (entityiterator instanceof CartelevatorWestEntity) {
					{
						Entity _ent = entity;
						_ent.teleportTo((entityiterator.getX()), (entityiterator.getY() - 1), (entityiterator.getZ()));
						if (_ent instanceof ServerPlayer _serverPlayer)
