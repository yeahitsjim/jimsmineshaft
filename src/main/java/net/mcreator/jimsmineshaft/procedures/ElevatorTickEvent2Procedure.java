...
public class ElevatorTickEvent2Procedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (("down").equals(entity.getPersistentData().getString("movement"))) {
			...
			entity.getPersistentData().putBoolean("doorOpen", false);
		}
			...
			if (entity.getY() < 20) {
				{
					...
					for (Entity entityiterator : world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(10 / 2d), e -> true).stream().sorted(Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_center))).toList()) {
						if (entityiterator instanceof Player) {
							if (entityiterator.getData(JimsmineshaftModVariables.PLAYER_VARIABLES).darknessOverlay < 60) {
		...
		if (ResourceKey.create(Registries.DIMENSION, ResourceLocation.parse("jimsmineshaft:level_1_dimension")) == (entity.level().dimension())) {
			if (entity.getY() <= 217) {
				if (entity.getPersistentData().getBoolean("needsFirstStop") == true) {
					entity.getPersistentData().putBoolean("needsFirstStop", false);
					entity.getPersistentData().putBoolean("hasOpenedTheDoorForYou", true);
					...
					entity.getPersistentData().putString("movement", "null");
					entity.getPersistentData().putString("stage", "lvl1end");
			...
			if (entity.getY() > 210 && entity.getY() < 220) {
				{
					...
					for (Entity entityiterator : world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(10 / 2d), e -> true).stream().sorted(Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_center))).toList()) {
						if (entityiterator instanceof Player) {
							if (entityiterator.getData(JimsmineshaftModVariables.PLAYER_VARIABLES).darknessOverlay > -40) {
								{
									JimsmineshaftModVariables.PlayerVariables _vars = entityiterator.getData(JimsmineshaftModVariables.PLAYER_VARIABLES);
									_vars.darknessOverlay = entityiterator.getData(JimsmineshaftModVariables.PLAYER_VARIABLES).darknessOverlay - 1;
									_vars.syncPlayerVariables(entityiterator);
			...
			if (entity.getY() < 203) {
				entity.getPersistentData().putString("movement", "null");
				{
					...
					for (Entity entityiterator : world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(10 / 2d), e -> true).stream().sorted(Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_center))).toList()) {
						...
						if (JimsmineshaftModVariables.MapVariables.get(world).currentLevel == 2) {
							if (entityiterator instanceof ServerPlayer _player && !_player.level().isClientSide()) {
						...
						if (JimsmineshaftModVariables.MapVariables.get(world).currentLevel == 3) {
							if (entityiterator instanceof ServerPlayer _player && !_player.level().isClientSide()) {
						...
						if (JimsmineshaftModVariables.MapVariables.get(world).currentLevel == 4) {
							if (entityiterator instanceof ServerPlayer _player && !_player.level().isClientSide()) {
						...
						if (JimsmineshaftModVariables.MapVariables.get(world).currentLevel == 5) {
							if (entityiterator instanceof ServerPlayer _player && !_player.level().isClientSide()) {
						...
						if (JimsmineshaftModVariables.MapVariables.get(world).currentLevel == 6) {
							if (entityiterator instanceof ServerPlayer _player && !_player.level().isClientSide()) {
