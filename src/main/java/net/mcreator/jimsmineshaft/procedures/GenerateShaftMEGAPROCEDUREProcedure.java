...
@EventBusSubscriber
public class GenerateShaftMEGAPROCEDUREProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		...
		boolean killProcedure = false;
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == JimsmineshaftModItems.WOE_MINESHAFT_BE_UPON_THE.get()) {
			JimsmineshaftModVariables.MapVariables.get(world).totalTokensRemaining = 60;
			JimsmineshaftModVariables.MapVariables.get(world).syncData(world);
			JimsmineshaftModVariables.MapVariables.get(world).levelTokens = 60;
			JimsmineshaftModVariables.MapVariables.get(world).syncData(world);
			...
			if (!world.isClientSide() && world.getServer() != null)
				world.getServer().getPlayerList().broadcastSystemMessage(Component.literal("\u00A76\u00A7l=========Beginning Structure Gen========="), false);
			if (!world.isClientSide() && world.getServer() != null)
				world.getServer().getPlayerList().broadcastSystemMessage(Component.literal("\u00A76\u00A7l======================================="), false);
			...
			if (0 < JimsmineshaftModVariables.MapVariables.get(world).totalTokensRemaining) {
				if (0 < JimsmineshaftModVariables.MapVariables.get(world).levelTokens) {
					while (JimsmineshaftModVariables.MapVariables.get(world).totalTokensRemaining > 0) {
						if (killProcedure == true) {
							break;
						}
						if (currentSavedBranchCount >= 4) {
							if (!world.isClientSide() && world.getServer() != null)
								world.getServer().getPlayerList().broadcastSystemMessage(Component.literal("\u00A74Branch cap reached, killing procedure. Coords: X: "), false);
							killProcedure = true;

							CallableGenerateShaftProcedure.execute(world);
						} else {
							if (!world.isClientSide() && world.getServer() != null)
							...
							if (!world.isClientSide() && world.getServer() != null)
								world.getServer().getPlayerList()
										.broadcastSystemMessage(Component.literal(("\u00A76\u00A7ISetting working coords to: X: " + (new java.text.DecimalFormat("##.##").format(workingCoords.get("x").getAsDouble()) + ""
