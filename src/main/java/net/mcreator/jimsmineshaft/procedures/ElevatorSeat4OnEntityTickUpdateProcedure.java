...
public class ElevatorSeat4OnEntityTickUpdateProcedure {
			...
			return;
		{
			...
			for (Entity entityiterator : world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(8 / 2d), e -> true).stream().sorted(Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_center))).toList()) {
				if (entityiterator instanceof ElevatorEntity) {
					{
						Entity _ent = entity;
						_ent.teleportTo((entityiterator.getX() + 1), (entityiterator.getY() + 0), (entityiterator.getZ() + -1.7));
						if (_ent instanceof ServerPlayer _serverPlayer)
							_serverPlayer.connection.teleport((entityiterator.getX() + 1), (entityiterator.getY() + 0), (entityiterator.getZ() + -1.7), _ent.getYRot(), _ent.getXRot());
					}
				}
			}
		}
